...
spring = st.sidebar.number_input('Spring Rate (lb/in)', value = 450, step=25)#lb/in
...
dr = (0.05, 0.10,0.2,0.3,0.4,0.5) #damping ratios
...
with row1_2:
    ...
    st.text(f'Joules per g = {np.round(jpg,1)}')
...
with row1_3:
    st.text(f'Metric Spring Rate = {np.round(mspring, 1)}N/mm')
